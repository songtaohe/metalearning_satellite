...
class MAML(object):
	def __init__(self, sess, num_test_updates = 20, inner_lr = 0.001):
		...
		with tf.variable_scope("foo", reuse=False):
			self.task_losses, self.task_outputs, _, self.groupA_loss,self.groupA_losses,self.max_grad = CNNmodel.buildMetaBlockV1(self.inputA, self.outputA, self.inputB, self.outputB, inner_step = self.num_updates, inner_lr = self.inner_lr)
		with tf.variable_scope("foo", reuse=True):
			self.task_losses_test, self.task_test_outputs, self.debug_inner_output, self.groupA_loss_test,_,_ = CNNmodel.buildMetaBlockV1(self.inputA, self.outputA, self.inputB, self.outputB, inner_step = self.num_test_updates, inner_lr = self.inner_lr_test, layer_st = 10, layer_ed = 13)
		...
		self.metatrain_groupA_op = self.opt.apply_gradients(grads) 
	...
	def addLog(self, test_loss, train_loss, lr,g1,g2):
...
if __name__ == "__main__":
	...
	name = "ModelV1_20180502_multidataset_osm_deepglobal_dstl_fromscretch_groupA"
	run = name+"_run6"
	...
	with tf.Session() as sess:
		...
		while True:
			...
			ret = model.trainModelGroupA(iA,oA, grad_scale)

			g1 = ret[0]
			g2 = ret[1]

			new_scale = g2/g1
			if new_scale > grad_scale:
				grad_scale = grad_scale * 0.95 + 0.05 * new_scale
			else:
				grad_scale = grad_scale * 0.5 + 0.5 * new_scale
			...
			if step % 200 == 0 or step == 1:
				...
				print("max train grad",g1,"max inner grad", g2)

				if step>1:
					summary = model.addLog(test_loss, train_loss, model.meta_lr_val, sum_g1/200, sum_g2/200)
					writer.add_summary(summary, step)

					#new_scale = sum_g2 / sum_g1 

					#grad_scale = grad_scale * 0.6 + 0.4 * new_scale

					#print("Update Scale to ", grad_scale)
