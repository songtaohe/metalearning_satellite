...
class MAML(object):
	def __init__(self, sess, num_test_updates = 20, inner_lr = 0.001):
		...
		with tf.variable_scope("foo", reuse=True):
			self.baseline_output_, _ = CNNmodel.build_unet512_12_V1(self.inputA, prefix="first_", deconv=True)
...
if __name__ == "__main__":
	...
	name = "ModelV1_20180406_baseline_road_usa_inputfilter"
	...
	with tf.Session() as sess:
		...
		dataloader1 = DataLoader('/data/songtao/metalearning/dataset/global/boston/', 36, 5, preload = False)
		dataloader2 = DataLoader('/data/songtao/metalearning/dataset/global/la/', 36, 5, preload = False)
		dataloader3 = DataLoader('/data/songtao/metalearning/dataset/global/chicago/', 36, 5, preload = False)
		...
		#testCases = dataloader2.getTestBatchs(20,20)

		# testCase = []
		# for i in xrange(2):
		# 	testCase.append(dataloader.getBatch(20,10,update = True))
		...
		for i in xrange(20):
			...
			testCase.append(dataloader.getTestBatchs(20,1))
		...
		while True:
			#if step % 5 == 0:
			iA,oA, iB, oB, task_id = dataloader.getBatch(20,1,update = True)
			#else:
			#	iA,oA, iB, oB, task_id = dataloader.getBatch(20,1,update = False)
			#print(step)
			


			ret = model.trainBaselineModel(iA,oA, task_lr[task_id])
			...
			if step % 200 == 0 or step == 1:
				...
				for testCases in tmpTestCase:
					#for i in xrange(5):
					for i in [baseline_task_id ]:
						

						testInputs = [testCases[0][i],testCases[1][i], testCases[2][i],testCases[3][i],i]
						#print(testInputs)
						result, result_loss = model.runBaselineModel(testInputs[0],testInputs[1])
						...
						if eeid < 2:
							...
							test_loss_first_two = test_loss_first_two + result_loss

						tmp_img = np.zeros((512,512,3), dtype=np.uint8)
						tmp_img2 = np.zeros((512,512), dtype=np.uint8)

						for ind in xrange(len(testInputs[0])):
							tmp_img = (testInputs[0][ind,:,:,:]*255).reshape(512,512,3).astype(np.uint8)
							Image.fromarray(tmp_img).save(output_folder1+'img_%d_%d_sat.png' % (eeid, ind))
							#if ind < 5:
							#	print(np.amin(result[ind,:,:,:]*255), np.amax(result[ind,:,:,:]*255))
							...
							tmp_img2 = (testInputs[1][ind,:,:,0]*255).reshape(512,512).astype(np.uint8)
							Image.fromarray(tmp_img2).save(output_folder1+'img_%d_%d_task%d_target.png' % (eeid, ind, testInputs[4]))
							
					eeid += 1

				test_loss/=len(tmpTestCase)
				test_loss_first_two /= 2 

				print("test_loss", test_loss_first_two, test_loss)

				summary = model.addLog(test_loss_first_two, train_loss, model.meta_lr_val)
				...
				if step % 1000 == 0:
					...
					if longterm_loss > last_longterm_loss*1.0 and step > 8000:
