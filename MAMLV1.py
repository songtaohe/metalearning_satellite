...
class MAML(object):
	def __init__(self, sess, num_test_updates = 20, inner_lr = 0.001):
		...
		with tf.variable_scope("foo", reuse=True):
			self.task_losses_test, self.task_test_outputs, self.debug_inner_output = CNNmodel.buildMetaBlockV1(self.inputA, self.outputA, self.inputB, self.outputB, inner_step = self.num_test_updates, inner_lr = self.inner_lr)
	...
	def addLog(self, test_loss, train_loss, lr):
		return self.sess.run(self.summary_loss, feed_dict = {self.test_loss:test_loss, self.train_loss: train_loss, self.lr:lr})
	...
	with tf.Session() as sess:
		...
		while True:
			...
			if step % 200 == 0 or step == 1:
				...
				if step % 400 == 0:
					print("longterm_loss",longterm_loss/2," lr is", model.meta_lr_val)
				...
				writer.add_summary(summary[1], step)
				writer.add_summary(summary[2], step)
